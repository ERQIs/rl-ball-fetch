...
ResidualBlock = future_module.ResidualBlock
compute_losses = future_module.compute_losses
...
class FutureWithVisualDecoderAdapterModel(nn.Module):
    def __init__(self, future_cfg: ModelConfig, spatial_cfg: MultiScaleSpatialPriorConfig):
        ...
        self.adapter1 = ConvAdapter(future_cfg.s1, spatial_cfg.c1)
        self.adapter2 = ConvAdapter(future_cfg.s2, spatial_cfg.c2)
        self.adapter3 = ConvAdapter(future_cfg.s3, spatial_cfg.c3)

        if spatial_cfg.decoder_mode == "pyramid":
            self.visual_decoder = SpatialPyramidDecoder(spatial_cfg)
        else:
            self.visual_decoder = F3Decoder(spatial_cfg.c3, spatial_cfg.in_channels)

    def _probe(self, h3: torch.Tensor):
        g = h3.mean(dim=(2, 3))
        return self.pos_head(g), self.vel_head(g)

    def _decode_future(self, h1: torch.Tensor, h2: torch.Tensor, h3: torch.Tensor) -> torch.Tensor:
        if isinstance(self.visual_decoder, SpatialPyramidDecoder):
            a1 = self.adapter1(h1)
            a2 = self.adapter2(h2)
            a3 = self.adapter3(h3)
            return self.visual_decoder(a1, a2, a3)
        a3 = self.adapter3(h3)
        return self.visual_decoder(a3)

    def freeze_visual_decoder(self):
        for p in self.visual_decoder.parameters():
    ...
    def forward(self, frames: torch.Tensor):
        ...
        for _ in range(self.future_cfg.future_len):
            ...
            pred_future_frames.append(self._decode_future(rh1, rh2, rh3))
            p, v = self._probe(rh3)
            pred_pos.append(p)
            pred_vel.append(v)
        ...
        return {
            ...
            "pred_future_frames": torch.stack(pred_future_frames, dim=1),
        }
...
def build_spatial_cfg(cfg):
    ...
    return MultiScaleSpatialPriorConfig(
        ...
        c2=mdl.get("c2", 8),
        c3=mdl.get("c3", 8),
        decoder_mode=spatial.get("decoder_mode", "f3"),
        lambda_rec=1.0,
        lambda_trans_f2=0.15,
        lambda_trans=1.0,
        lambda_wd=1.0,
        lambda_nb=0.1,
        foreground_weight=6.0,
        foreground_dark_threshold=0.55,
        foreground_dilate_kernel=5,
    )
...
def maybe_load_spatial_init(model, cfg, device):
    ...
    dec_missing, dec_unexpected = model.visual_decoder.load_state_dict(decoder_state, strict=False)
    print("loaded shared visual decoder init from", ckpt_path)
    print("decoder missing keys:", list(dec_missing))
    print("decoder unexpected keys:", list(dec_unexpected))
    if bool(cfg["model"].get("freeze_visual_decoder", True)):
        model.freeze_visual_decoder()
        print("froze shared visual decoder parameters")
    return ckpt_path
...
@torch.no_grad()
def evaluate(model, loader, future_cfg, device):
    model.eval()
    agg = {"loss": 0.0, "frame": 0.0, "pos": 0.0, "vel": 0.0, "n": 0}
    for batch in loader:
        batch = {k: v.to(device) if hasattr(v, "to") else v for k, v in batch.items()}
        out = model(batch["frames"])
        _, stats = compute_losses(out, batch, future_cfg)
...
def save_curve_svg(history, out_path):
    ...
    svg = [
        ...
        f'<text x="{width/2:.0f}" y="20" text-anchor="middle" class="title">Adapter Future Pretrain Loss Curve</text>',
    ]
    ...
    svg.extend(
        [
            ...
            f'<line x1="{width-mr-160}" y1="{mt+18}" x2="{width-mr-136}" y2="{mt+18}" class="train" />',
            f'<text x="{width-mr-128}" y="{mt+22}">train_loss</text>',
            f'<line x1="{width-mr-160}" y1="{mt+42}" x2="{width-mr-136}" y2="{mt+42}" class="val" />',
            f'<text x="{width-mr-128}" y="{mt+46}">val_loss</text>',
            "</svg>",
        ]
    )
...
def main():
    ...
    print("started future pretraining with shared visual decoder adapter")
    print("device:", device)
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        for batch_idx, batch in enumerate(train_loader, start=1):
            batch = {k: v.to(device) if hasattr(v, "to") else v for k, v in batch.items()}
            out = model(batch["frames"])
            loss, stats = compute_losses(out, batch, future_cfg)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            for key in ["loss", "frame", "pos", "vel"]:
        ...
        train_stats = {k: agg[k] / max(agg["n"], 1) for k in ["loss", "frame", "pos", "vel"]}
        val_stats = evaluate(model, val_loader, future_cfg, device)
        row = {
            "epoch": epoch,
            "train_loss": train_stats["loss"],
            "train_frame": train_stats["frame"],
            "train_pos": train_stats["pos"],
            "train_vel": train_stats["vel"],
        ...
        print(
            ...
            f"val_loss={row['val_loss']:.4f} val_frame={row['val_frame']:.4f} "
            f"val_pos={row['val_pos']:.4f} val_vel={row['val_vel']:.4f}"
        )
