...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot downstream comparison curves including adapter transfer.")
    parser.add_argument("--repo-root", type=Path, default=Path("d:/projects/rl-ball-fetch"))
    parser.add_argument(
        "--stacked-root",
        type=Path,
        default=Path("d:/projects/rl-ball-fetch/temperal/experiments/continuity_prior/output/stacked_transfer_888_20260325"),
    )
    parser.add_argument(
        "--adapter-root",
        type=Path,
        default=Path("d:/projects/rl-ball-fetch/temperal/experiments/continuity_prior/output/adapter_transfer_20260327"),
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path(
            "d:/projects/rl-ball-fetch/temperal/experiments/continuity_prior/output/adapter_transfer_20260327/adapter_transfer_vs_baselines_curves.png"
        ),
    )
...
def draw_band(ax, epochs: np.ndarray, mean: np.ndarray, std: np.ndarray, color: str, label: str, alpha: float = 0.16):
...
def find_run_dirs(root: Path, method_tag: str, fraction_pct: int, date_tag: str) -> list[Path]:
    pattern = f"multiscale_transfer_{method_tag}_{fraction_pct}pct_obs50_seed*_{date_tag}"
...
def main() -> None:
    ...
    methods = {
        ...
        "adapter": {"label": "Adapter Finetune", "color": "#7c3aed"},
    }
    ...
    for col_idx, fraction in enumerate(fractions):
        ...
        stacked_dirs = find_run_dirs(args.stacked_root, "stacked888_finetune", fraction, "20260325")
        adapter_dirs = find_run_dirs(args.adapter_root, "adapter_finetune", fraction, "20260327")
        if not stacked_dirs:
            raise SystemExit(f"No stacked run dirs found for {fraction}% under {args.stacked_root}")
        if not adapter_dirs:
            raise SystemExit(f"No adapter run dirs found for {fraction}% under {args.adapter_root}")
        stacked_hist = aggregate_histories(stacked_dirs, args.epochs)
        adapter_hist = aggregate_histories(adapter_dirs, args.epochs)

        histories = {
            "scratch": scratch_hist,
            "future": future_hist,
            "stacked": stacked_hist,
            "adapter": adapter_hist,
        }
    ...
    fig.suptitle("Downstream Comparison: Scratch vs Future vs Stacked888 vs Adapter", fontsize=14)
