...
def read_history(path):
    ...
    for row in rows:
        parsed.append({
            ...
            'val_success': float(row['val_success']) if row.get('val_success') else None,
        })
    return parsed
...
def main():
    ...
    p.add_argument('--run', nargs=2, action='append', metavar=('CONFIG', 'RUN_DIR'), required=True)
    args = p.parse_args()
    ...
    for config_path, run_dir in args.run:
        ...
        test_metrics = evaluate(config_path, str(run_path / 'best.pt'), split='test')
        summary_rows.append({
            ...
            'val_success': best_row['val_success'],
            'test_mean_l2': test_metrics['mean_l2'],
            'test_median_l2': test_metrics['median_l2'],
            'test_success_at_0.2': test_metrics.get('success_at_0.2'),
        })
    ...
    for idx in range(epoch_count):
        ...
        aggregate_history.append({
            ...
            'val_success_mean': mean([row['val_success'] for row in rows if row['val_success'] is not None]),
            'val_success_std': std([row['val_success'] for row in rows if row['val_success'] is not None]),
        })

    summary_stats = [{
        'num_seeds': len(summary_rows),
        'val_mean_l2_mean': mean([row['val_mean_l2'] for row in summary_rows]),
        'val_mean_l2_std': std([row['val_mean_l2'] for row in summary_rows]),
        'val_success_mean': mean([row['val_success'] for row in summary_rows]),
        'val_success_std': std([row['val_success'] for row in summary_rows]),
        'test_mean_l2_mean': mean([row['test_mean_l2'] for row in summary_rows]),
        'test_mean_l2_std': std([row['test_mean_l2'] for row in summary_rows]),
        'test_success_mean': mean([row['test_success_at_0.2'] for row in summary_rows]),
        'test_success_std': std([row['test_success_at_0.2'] for row in summary_rows]),
    }]

    save_csv(
        summary_rows,
        out_dir / 'per_seed_summary.csv',
        ['run_dir', 'config', 'best_epoch', 'val_loss', 'val_mean_l2', 'val_median_l2', 'val_success', 'test_mean_l2', 'test_median_l2', 'test_success_at_0.2'],
    )
    save_csv(
        summary_stats,
        out_dir / 'aggregate_summary.csv',
        ['num_seeds', 'val_mean_l2_mean', 'val_mean_l2_std', 'val_success_mean', 'val_success_std', 'test_mean_l2_mean', 'test_mean_l2_std', 'test_success_mean', 'test_success_std'],
    )
    save_csv(
        aggregate_history,
        out_dir / 'aggregate_history.csv',
        ['epoch', 'train_loss_mean', 'train_loss_std', 'val_loss_mean', 'val_loss_std', 'val_success_mean', 'val_success_std'],
    )
