...
from src.models.multiscale_continuity_pretrain import (
    ContinuityConfig,
    MultiScaleContinuityPretrainer,
    compute_continuity_losses,
)
...
def evaluate(model, loader, model_cfg, device):
    model.eval()
    agg = {"l_total": 0.0, "l_rec": 0.0, "l_trans": 0.0, "l_wd": 0.0, "l_nb": 0.0, "n": 0}
    for batch in loader:
        batch = {k: v.to(device) if hasattr(v, "to") else v for k, v in batch.items()}
        outputs = model(batch["i_t"], batch["i_t1"], batch["flow_t"])
        _, stats = compute_continuity_losses(outputs, batch["i_t"], batch["i_t1"], model_cfg)
        for key in ["l_total", "l_rec", "l_trans", "l_wd", "l_nb"]:
            agg[key] += stats[key]
        agg["n"] += 1
    for key in ["l_total", "l_rec", "l_trans", "l_wd", "l_nb"]:
        agg[key] /= max(agg["n"], 1)
...
def save_history(history, out_dir):
    ...
    fieldnames = ["epoch", "train_total", "train_rec", "train_trans", "train_wd", "train_nb", "val_total", "val_rec", "val_trans", "val_wd", "val_nb"]
...
def save_curve_svg(history, out_path):
    ...
    svg = [
        ...
        f'<text x="{width/2:.0f}" y="20" text-anchor="middle" class="title">Continuity Pretrain Loss Curve</text>',
    ]
...
def main():
    ...
    model = MultiScaleContinuityPretrainer(model_cfg).to(device)
    ...
    for epoch in range(1, epochs + 1):
        model.train()
        agg = {"l_total": 0.0, "l_rec": 0.0, "l_trans": 0.0, "l_wd": 0.0, "l_nb": 0.0, "n": 0}
        running = {"l_total": 0.0, "l_rec": 0.0, "l_trans": 0.0, "l_wd": 0.0, "l_nb": 0.0, "n": 0}
        total_batches = len(train_loader)
        print(f"epoch {epoch:03d} started | batches={total_batches} | device={device}")
        for batch_idx, batch in enumerate(train_loader, start=1):
            batch = {k: v.to(device) if hasattr(v, "to") else v for k, v in batch.items()}
            outputs = model(batch["i_t"], batch["i_t1"], batch["flow_t"])
            loss, stats = compute_continuity_losses(outputs, batch["i_t"], batch["i_t1"], model_cfg)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            for key in ["l_total", "l_rec", "l_trans", "l_wd", "l_nb"]:
            ...
            if batch_idx == 1 or batch_idx % log_every == 0 or batch_idx == total_batches:
                ...
                print(
                    ...
                    f"trans={running['l_trans']/denom:.4f} wd={running['l_wd']/denom:.4f} nb={running['l_nb']/denom:.4f}"
                )
                running = {"l_total": 0.0, "l_rec": 0.0, "l_trans": 0.0, "l_wd": 0.0, "l_nb": 0.0, "n": 0}
        ...
        print(
            ...
            f"rec={val_stats['l_rec']:.4f} trans={val_stats['l_trans']:.4f} wd={val_stats['l_wd']:.4f} nb={val_stats['l_nb']:.4f}"
        )
        ...
        ckpt = {
            ...
            "history": history,
            "config": cfg,
            "continuity_config": asdict(model_cfg),
        }
